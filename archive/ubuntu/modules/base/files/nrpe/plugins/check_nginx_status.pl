...
-m, --metric=Active,ReqPerSec,ConnPerSec,ReqPerConn
   the metric to collect. For Active we also get the Writing, Reading, Waiting
...
sub check_options {
    ...
    GetOptions(
      ...
      'm:s'   => \$o_metric,       'metric:s'      => \$o_metric,
...
if ($response->is_success) {
    ...
    my $TempFile = $TempPath.$o_host.'_check_nginx_status_'.$o_metric.'_'.md5_hex($url);
    my $FH;
    ...
    if($o_metric eq 'Active') {
	$PerfData = sprintf ("Active=%d Writing=%d Reading=%d Waiting=%d"
			     ,($ActiveConn), ($Writing),($Reading),($Waiting));
    } elsif($o_metric eq 'ReqPerSec') {
	$PerfData = sprintf ("ReqPerSec=%f", ($ReqPerSec));
    } elsif($o_metric eq 'ConnPerSec') {
	$PerfData = sprintf ("ConnPerSec=%f", ($ConnPerSec));
    } elsif($o_metric eq 'ReqPerConn') {
	$PerfData = sprintf ("ReqPerConn=%f", ($ReqPerConn));
    } else {
